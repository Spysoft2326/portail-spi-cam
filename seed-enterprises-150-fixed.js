...
async function main() {
  ...
  console.log(`📊 Nombre d'entreprises à traiter : ${enterprises.length}`);

  // Vérification que Prisma est bien initialisé
  console.log("🔍 Vérification de Prisma Client...");
  const models = Object.keys(prisma).filter(k => !k.startsWith('_') && !k.startsWith('$'));
  console.log("   Modèles disponibles :", models.join(", "));

  // En Prisma Client JS, model Entreprise -> prisma.entreprise (camelCase)
  if (!prisma.entreprise) {
    console.error("❌ ERREUR CRITIQUE : Le modèle 'entreprise' n'existe pas dans Prisma Client !");
    console.error("   Modèles trouvés :", models.join(", "));
    console.error("   Vérifiez que :");
    console.error("   1. Le modèle 'Entreprise' existe dans prisma/schema.prisma");
    console.error("   2. Vous avez exécuté 'npx prisma generate' après modification du schema");
    console.error("   3. Le nom du modèle est bien 'Entreprise' (E majuscule, avec s)");
    process.exit(1);
  }

  console.log("✅ Modèle 'entreprise' trouvé dans Prisma Client");
  console.log("");
  ...
  for (const enterprise of enterprises) {
    try {
      // Mapping des champs du seed vers le modèle Entreprise du schema
      const entrepriseData = {
        referenceSPI: `SPI-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`,
        denomination: enterprise.name,
        sigle: enterprise.name.match(/\(([A-Z]+)\)/)?.[1] || null,
        formeJuridique: "SA",
        capitalSocial: 1000000,
        dateCreation: new Date("2000-01-01"),
        numRegistreCommerce: `RCCM-${Math.random().toString(36).substring(2, 10).toUpperCase()}`,
        numContribuable: `M${Math.floor(Math.random() * 1000000000)}`,
        adresse: `${enterprise.city}, Cameroun`,
        ville: enterprise.city,
        departement: enterprise.region,
        region: enterprise.region.toUpperCase(),
        codePostal: null,
        latitude: null,
        longitude: null,
        telephone: null,
        email: null,
        siteWeb: enterprise.website || null,
        secteurActivite: enterprise.sector.toUpperCase(),
        sousSecteur: null,
        produitsPrincipaux: enterprise.description,
        statut: "ACTIF",
      ...
      const result = await prisma.entreprise.upsert({
        where: { denomination: enterprise.name },
        update: {
          ...entrepriseData,
          updatedAt: new Date(),
        },
      ...
      created++;
      console.log(`  ✅ ${enterprise.name} (${enterprise.sector})`);
    } catch (error) {
      errors++;
      console.error(`  ❌ Erreur : ${enterprise.name}`);
