...
export default withAuth(
  function middleware(req) {
    ...
    if (path.startsWith("/api/public") || path === "/" || path.startsWith("/entreprises") || path.startsWith("/notes-conjoncture")) {
      return NextResponse.next();
    }

    // Routes Super-Admin uniquement
    if (path.startsWith("/dashboard/super-admin") || path.startsWith("/parametres")) {
    ...
    if (path.startsWith("/dashboard/admin") || path.startsWith("/entreprises/manage") || path.startsWith("/conjoncture/write")) {
  ...
  },
  {
    callbacks: {
      authorized({ req, token }) {
        ...
        const publicPaths = ["/", "/entreprises", "/notes-conjoncture", "/api/public"];
